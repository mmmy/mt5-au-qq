...
程序启动时会自动读取项目根目录的 `.env` 文件。首次配置可以复制示例：

```powershell
Copy-Item .env.example .env
```

修改 `.env` 后需要重启服务。系统/PowerShell 环境变量的优先级高于 `.env`，因此仍可临时覆盖某个配置。`.env` 已被 Git 忽略，不应提交账户或部署机的私有配置；`.env.example` 用于记录可配置项和默认示例。
...
创建警报时可以选择开仓方向、时间级别和有效 K 线数。开始时间不在前端显示或提交，由后端自动设置为当前时间。未提供其他参数时默认方向为“自动”、周期为 2 分钟、有效期按一天折算。有效期沿用 Pine 策略现有的自然时间计算方式，因此包含休市时间。新警报的参数会按 TradingView `alert_id` 保存到 SQLite，并在警报列表中回显。
